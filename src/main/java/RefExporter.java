...
public class RefExporter {
	...
	// Sometimes, there can be a file which doesn't have any text with the chosen annotation.
	// And when processing that annotation, The exporter fails with NULL POINTER EXCEPTION
	// So, using the start  integer to do the check whether an annotation type exists in the for loop
	// This checks for every entry of the annotation. Should have a better way to deal with it
	// Something like IF EXISTS GESESTZ_TYPE, Then FOR loop
